...
def generate_long():
    try:
        ...
        tasks = load_tasks()
        tasks[task_id] = {'status': 'running', 'video_url': '', 'error': ''}
        save_tasks(tasks)
        t = threading.Thread(target=background_generate_long, args=(task_id, segments, num), daemon=True)
        t.start()
...
def background_generate_long(task_id, segments, num):
    ...
    try:
        for i, seg in enumerate(segments):
            ...
            st, url = wait_ark(tid)
            if st != 'succeeded':
                tasks[task_id] = {'status': 'failed', 'video_url': '', 'error': f'第{i+1}段失败'}
                save_tasks(tasks)
                return
            ...
            seg_urls.append(url)
        if num == 1:
            tasks[task_id] = {'status': 'succeeded', 'video_url': seg_urls[0], 'error': ''}
            save_tasks(tasks)
            return
        out = os.path.join(TEMP_DIR, f'{run_id}_final.mp4')
        concat(seg_files, out)
        for f in seg_files: os.remove(f)
        tasks[task_id] = {'status': 'succeeded', 'video_url': f'{request.url_root}api/video/serve/{out}', 'error': ''}
        save_tasks(tasks)
    except Exception as e:
        tasks[task_id] = {'status': 'failed', 'video_url': '', 'error': str(e)}
        save_tasks(tasks)
...
def status(task_id):
    ...
    tasks = load_tasks()
    if task_id in tasks:
        return jsonify({'status': tasks[task_id]['status'], 'video_url': tasks[task_id]['video_url'], 'error': tasks[task_id]['error']})
...
def ark_poll(task_id):
    req = urllib.request.Request(f'{ARK_BASE}/api/v3/contents/generations/tasks/{task_id}', headers={'Authorization': f'Bearer {ARK_KEY}'}, method='GET')
    with urllib.request.urlopen(req, timeout=30, context=ssl._create_unverified_context()) as r:
        t = json.loads(r.read())
